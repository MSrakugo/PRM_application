...
def predict_cv_metamorphic(Models, feature_setting, Fold_num, train_x):
    ...
    return pred_train, feature_name

#################################################モデルの定義

def predict_model(elem, X_use, y,  path_all_share, path_figure_all, path_all_models, mobile_data_compile, spidergram_data_compile):
    ...
    try:
        ...
        print("/Model_setting.pkl maybe not exist")
    ...
    Fold_num = 4
    pred_y_log, feature_name = predict_cv_metamorphic(Models_predict, feature_setting, Fold_num, X_use)
    ...
    return pred_data, mobile_amount_data, mobile_data_compile, spidergram_data_compile


def model_main(mobile_elem, immobile_elem, Raw_metamorphic_rock, Raw_metamorphic_rock_location, model_folder_name, mobile_data_compile, spidergram_data_compile):
    ########################################################元素の数を計算
    #不動元素
    #一回全て代入
    ...
    ##########################################################################モデルによる推定
    ########################################MODEL
    pred_data, mobile_amount_data, mobile_data_compile, spidergram_data_compile = predict_model(elem, X_use, y,  path_all_share, path_figure_all, path_all_models, mobile_data_compile, spidergram_data_compile)
    ########################################MODEL  fin

    return mobile_data_compile, spidergram_data_compile 
...
def predict_protolith(mobile_elem_list, immobile_elem, Raw_metamorphic_rock, Raw_metamorphic_rock_location, now_model_folder_name):
    ...
    # 推定した値を入れる
    spidergram_data_compile = pd.DataFrame(index = Raw_metamorphic_rock.index, columns = mobile_elem_list)
    ...
    for mobile_elem in mobile_elem_list:
        ...
        try:
            mobile_data_compile, spidergram_data_compile = model_main(mobile_elem, immobile_elem, Raw_metamorphic_rock, Raw_metamorphic_rock_location, now_model_folder_name, mobile_data_compile, spidergram_data_compile)
        except:
            pass
